...
def upload_resume(req: ResumeUploadRequest, deps=Depends(get_deps)):
    try:
        ...
        kb_key, cfg = _resolve_user_upload_kb(deps, req.app_id, req.kb_key)
        collection = _ensure_collection(deps, cfg)

        resume_id = (req.resume_id or "").strip() or str(uuid.uuid4())
        ...
        if req.metadata:
            combined = {"resume": req.resume, "metadata": req.metadata}
            props["metadata_json"] = _serialize_payload(combined)
        if cfg.get("use_allowed_apps_filter"):
            props["allowed_apps"] = req.app_id

        vector = deps.embedding_client.embed_one(resume_text, app_id=req.app_id)
